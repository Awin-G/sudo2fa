...
- 外部辅助程序一律通过绝对路径调用——绝不走 `PATH`——防止 setuid
  进程被劫持。
- token 共 12 字节——4 字节过期时间 + 8 字节截断的 HMAC-SHA1——编码为 16 个
  URL 安全的 base64 字符。绑定的父进程 pid 不落盘、而是签名进 MAC（0 表示
  不绑定），比对为常量时间。
- 失败尝试全局限速：一次验证码或 token 失败后开启 3 秒窗口，窗口内后续尝试
  在验证前一律拒绝。时间戳存放在密钥文件旁的 root 所有、`0600` 状态文件
  （`/etc/shadow2fa.retry`）。
